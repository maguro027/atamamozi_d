...
public class Editer {
  ...
  public static void setCheckPoint_Editr(Player player) {
    if (!CheckPoint_Editr.contains(player)) {
      player.sendMessage(waterpunch.atamamozi_d.plugin.tool.CollarMessage.setInfo() + ChatColor.WHITE + "Run " + "[" + ChatColor.RED + "/atamamozi_d setstartpoint [r]" + ChatColor.WHITE + "]" + " at your destination");
      player.closeInventory();
